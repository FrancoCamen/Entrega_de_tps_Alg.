...
def say_traje_armadura(lista):
    for i in range(lista.size()):
        ...
        if 'armadura' in heroe.biografia:
            print(f'en la biografia de {heroe.name} se encontro la palabra armadura')
        elif 'traje' in heroe.biografia:
            print(f'en la biografia de {heroe.name} se encontro la palabra traje')
...
def first_b_m_s(lista):
    for i in range(lista.size()):
        heroe = lista.get_element_by_index(i)
        if heroe.name[0] == 'B':
            print(f'La primer letra de {heroe.name} es B')
        if heroe.name[0] == 'M':
            print(f'La primer letra de {heroe.name} es M')
        if heroe.name[0] == 'S':
            print(f'La primer letra de {heroe.name} es S')
        else:
...
def antes_de_un_año(lista):
    for i in range(lista.size()):
        heroe = lista.get_element_by_index(i)
        if heroe.year < 1963:
            print(f'El personaje {heroe.name} de la casa cinematografica {heroe.empresa} aparecio antes de 1963')
        elif heroe.year == 1963:
            print(f'El personaje {heroe.name} aparecio en el año 1963')
                
        else:
            print(f'El personaje {heroe.name} aparecio luego de 1963')
                
# f. mostrar la casa a la que pertenece Capitana Marvel
def house_woman_amazing_capitana_marvel(lista):
    indice = lista.search_r("Capitana Marvel", 0, lista.size()-1, "name")
    if indice != None:
            new_house = lista.get_element_by_index(indice)
            print(f'Capitana Marvel es de {new_house.empresa}')

    
# g. mostrar toda la información de Flash y Star-Lord;
def info_flash_star(lista):
    indice = lista.search_r("Flash", 0, lista.size()-1, "name")
    if indice != None:
        new_house = lista.get_element_by_index(indice)
        print(f'Biografia de Flash: {new_house.biografia}')

    indice2 = lista.search_r("Star-Lord", 0, lista.size()-1, "name")
    if indice2 != None:
...
def dc_marvel(lista):
    ...
    for i in range(lista.size()):
        heroe = lista.get_element_by_index(i)
        if heroe.empresa == 'Marvel':
            cont += 1
    print(f'Marvel tiene {cont} personajes en la lista')

remove_GreenLantern(sh)
get_year_Wolverine(sh)
change_DrStrange(sh, "Dr. Strange", "Nueva casa", "name")
print(f"Los superheores con traje o armadura en su biografia son {search_suitORarmor(sh)}")
first_b_m_s(sh)
antes_de_un_año(sh)
house_woman_amazing_capitana_marvel(sh)
info_flash_star(sh)
dc_marvel(sh)
dc_marvel(sh)
